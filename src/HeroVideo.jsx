...
export default function HeroVideo() {
  return (
    <div className="w-full h-full relative bg-black">
      ...
      {/* Header */}
      <header className="absolute top-0 left-0 w-full flex items-center justify-between px-10 pt-8 z-10">
        ...
        <nav className="flex items-center gap-10">
          {['Contact', 'News', 'About'].map((label) => (
            <button
              key={label}
              className="font-serif text-white/75 text-xs tracking-[0.2em] uppercase
      ...
      {/* Footer */}
      <footer className="absolute bottom-0 left-0 w-full px-10 pb-20 z-10">
        <ScrollFloat
          as="p"
          animationDuration={1}
          ease="back.inOut(2)"
